...
describe('ApiGetData', () => {
  ...
  it('AddData & GetData', async () => {
    // Get data before add
    const ret1 = await client.callApi('GetData', {})
    assert.strictEqual(ret1.isSucc, true)

    // AddData
    const ret2 = await client.callApi('AddData', { content: 'AABBCC' })
    assert.strictEqual(ret2.isSucc, true)

    // Get data again, the new data should appear
    const ret3 = await client.callApi('GetData', {})
  ...
  it('AddData: Check content is empty', async () => {
